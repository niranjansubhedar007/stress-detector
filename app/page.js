...
export default function Home() {
  ...
  const detectEmotion = () => {
    // Simulate different emotions randomly
    const emotions = ["happy", "angry", "sad", "surprised", "fearful", "disgusted", "neutral"];
    const randomEmotion = emotions[Math.floor(Math.random() * emotions.length)];
    
    setEmotion(randomEmotion);
    console.log(`Detected emotion: ${randomEmotion}`);
    
    // For debugging, you can also log the full expression probabilities
    ...
    console.log("Expression probabilities:", mockExpressions);
  };
  ...
  useEffect(() => {
    if (isDetecting) {
      ...
      detectionInterval.current = setInterval(detectEmotion, 2000); // Detect every 2 seconds
    } else {
    ...
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20">
      <main className="flex flex-col gap-8 row-start-2 items-center w-full max-w-4xl">
        <div className="flex flex-col items-center gap-4 w-full">
          ...
          />
          <h1 className="text-2xl font-bold">Stress Detection App</h1>
          <p className="text-gray-600">(Console logging mode)</p>
        </div>

        {isModelLoading ? (
          <div className="p-4 rounded-lg bg-gray-100 dark:bg-gray-800">
            Loading emotion detection models... (check console)
          </div>
        ) : (
          <div className="flex flex-col items-center gap-6 w-full">
            <div className="p-4 rounded-lg bg-gray-100">
              <p>Camera is disabled in this mode. Check browser console for emotion detection logs.</p>
            </div>
...
{/* {error && <p className="text-red-500">Camera access denied. Please allow camera permissions.</p>} */}
            {isDetecting && (
              <div className={`p-4 rounded-lg text-center w-full ${getEmotionColor()}`}>
                <h2 className="text-xl font-semibold">{getEmotionMessage()}</h2>
                <p className="text-sm mt-2">(Random simulated emotion - check console for details)</p>
              </div>
            )}
          </div>
        )}
